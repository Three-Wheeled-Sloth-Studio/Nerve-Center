...
Exact-head run `66e8264a-7599-412d-8c31-33f3b6620da2` then proved a separate scheduler-reachability limit: across 16 cycles, all network searches were civic employer-landscape queries and essentially all remaining capacity went to legacy company/source revisits. It completed 64 strategies, 16 successful searches, 77 successful scans, 24 inspected postings, and 10/10 full scores, but executed no reflected/direct role-search strategy and retained no new role. Checkpoint `ecddb58ab812f1c21ac940724e1cda13bfed1775` reserves one intent-aligned direct/adjacent/capability/reflection search only when excess company revisits would otherwise consume every unreserved slot; accepted local-market, employer-deepening, regional-probe, and company-revisit protections remain intact.

Run `18c4422c-5c72-4b07-b805-6f0a19821b39` immediately proved role-search reachability: 18 public searches covered nine local-employer and nine intent-aligned role families, returned 84 results with seven eligible, resolved one company and one career source, inspected 155 postings, and retained six openings. It surfaced a strong current remote target, Brigit `Head of Product`, with fit `66.2`. The run also exposed the last family-label escape: old or new non-archetype families could still carry an unsupported `Human-Centered Design company` anchor/archetype. Checkpoint `ab460a148586e20a053165c7a00673f346efdb41` validates every archetype constraint regardless of family and rejects unsupported employer-class anchors.

Final exact-head run `2a34d026-5470-487c-b9f3-221d01a88137` completed through planned `admission_draining` on `ab460a148586e20a053165c7a00673f346efdb41`: eight successful searches split evenly between local-market and adjacent-role discovery, 36 returned results, 48 inspected postings, 4/4 successful full scores, and zero malformed query/reflection matches. Durable malformed strategies remained auditable but consumed no scheduler slot or request. Application version remains `0.12.31`; schema remains 15. Full Ruff and 307 Python tests passed, as did refs/catalog validation and desktop web. Local Rust validation is environment-blocked because the installed Visual Studio C++ toolchain cannot find its own `excpt.h`; no Rust source changed.

Issue #65 remains the next planned core slice. The next Job Scout quality diagnostic, if prioritized before #65, is result-to-opening precision: the productive role lane can resolve large company feeds, but most fetched postings remain off-target and only the highest-evidence retained roles should reach the actionable shortlist.
