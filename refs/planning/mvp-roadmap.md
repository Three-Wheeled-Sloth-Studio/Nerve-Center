...
## Next Job Scout discovery-quality slice: source-aware query portfolios and gap-driven reflection

After Issue #40 is accepted, improve discovery experiment quality without replacing the durable company-first architecture.
...
6. **Human-visible strategy audit without mandatory approval.** Expose generated search hypotheses, source-specific compiled queries, observed yield, conditioned yield, warnings, and strategy weight changes so the user can understand and diagnose discovery. Normal unattended sessions should not require a human click before each search.
