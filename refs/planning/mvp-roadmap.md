...
Run `cf93ae1a-4958-4dee-9959-5fe671cf4eda` completed the gate with exact accounting: 175 requests, 33 LLM calls, 21/21 successful full scores, 26 public searches, 240 returned results, 13 inspected civic references, 12 added companies, 12 added sources, and 37 inspected postings. Volvo Group's official Greensboro career surface was refreshed with current-run provenance. Durham and Durham-Chapel Hill strategies executed and reached Google, PowerSecure, and SAS career surfaces, but not Wolfspeed.

The apparent two directory candidates were both page-level Greensboro Chamber publisher metadata. A general post-gate correction now admits schema.org organizations only within an `ItemList` or explicit employer/company field; it does not blacklist the Chamber or any employer. This leaves zero valid directory-derived employer candidates from the run. The next source-capability slice should follow and parse bounded public civic-directory attachments, including the observed PDF and tabular formats, with the same cache, provenance, and cooldown contract. Wolfspeed remains an acceptance probe rather than a production rule.
...
### Manager Attention and Review foundation (implemented)

Issue **#61: Add manager-owned Attention and Review queue foundation** is implemented in `0.12.28` with additive schema 13.

Accepted behavior:

1. The manager owns durable Attention/Review item identity, lifecycle, context, allowed dispositions, validation guidance, downstream meaning, dependency keys, and audit history.
2. Module submissions are idempotent by manager-owned module/idempotency identity; duplicate submissions do not duplicate human work.
3. Open items block only explicitly declared dependency keys. Unrelated module, session, queue, and synthetic work remains admissible.
4. Resolution and dismissal are explicit, attributable transitions with durable history.
5. Module-supplied context and dispositions are descriptive only and cannot grant capabilities, change Code Shop authority, or override manager risk decisions.
