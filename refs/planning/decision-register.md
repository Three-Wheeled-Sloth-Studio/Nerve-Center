...
| NC-047 | accepted | Job Scout fit is responsibility- and evidence-first rather than title-driven. Listed titles are weak clues only. Fit should compare actual requirements/work to verified career evidence, and domain relationship should rank direct domain match above adjacent domain, transferable experience, and domain/skill mismatch. |
| NC-048 | accepted | Code Shop, with stable module ID and storage namespace `code_shop`, is Nerve Center's second first-party reference module. It owns software-engineering orchestration but receives neither unrestricted shell/filesystem access nor provider/model authority. |
| NC-049 | accepted | Nerve Center may perform bounded unattended external engineering actions only when a manager-verified official module requests a declared capability, the user has explicitly allowed that action for the project, the action is attributable to durable work, all scope checks pass, and manager risk policy permits it. User-supplied modules cannot receive this autonomous authority. Deployment, credential or secret mutation, destructive Git, irreversible data changes, and equivalently high-risk actions remain prohibited or human-gated regardless of ordinary project write authority. |
| NC-050 | accepted | GitHub is Code Shop's source of truth for repository identity and repository metadata. Nerve Center stores user selection, lifecycle, priority, authority, and a machine-local checkout association keyed to stable GitHub identity. Users link discovered repositories to checkout folders; path containment and Git remote identity must be validated before local execution. |
| NC-051 | accepted | Privileged local engineering execution is a manager-owned typed capability boundary. Code Shop may request explicit repository, Git, shell, test, GitHub, research, workflow, and runner capabilities, but it cannot invoke a generic unrestricted shell or silently escape a registered checkout and approved root. |
