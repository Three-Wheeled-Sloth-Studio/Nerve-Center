#!/usr/bin/env python3
"""Generate deterministic OKF discovery indexes for the Nerve Center refs bundle."""
...
def concept_entry(path: Path) -> str:
    ...
    description = str(frontmatter.get("description") or "Nerve Center knowledge concept.")
    return f"* [{title}]({path.name}) - {description}"


def resource_entry(path: Path, kind: str) -> str:
    return f"* [{path.name}]({path.name}) - Nerve Center {kind} resource."
...
def render_index(directory: Path, okf_version: str, bundle_title: str) -> str:
    ...
    concepts = [path for path in files if path.suffix.lower() == ".md"]
    structured = [path for path in files if path.suffix.lower() in {".yaml", ".yml"}]
    support = [path for path in files if path not in concepts and path not in structured]
...
def expected_indexes() -> dict[Path, str]:
    ...
    title = str(bundle.get("title") or "Nerve Center Knowledge Bundle")
    if not version:
